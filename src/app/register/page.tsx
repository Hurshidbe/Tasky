...
import { Mail, Lock, ArrowRight, Loader2, Layout, AlertCircle } from "lucide-react";
      ...
      <div className="w-full max-w-[360px] bg-slate-900/40 backdrop-blur-2xl p-7 rounded-3xl border border-white/5 shadow-2xl z-10 animate-in fade-in zoom-in-95 duration-500">
        <div className="flex flex-col items-center mb-6">
          ...
          <p className="text-slate-500 text-xs font-medium text-center">
            Join the Tasky community today
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          >
            {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <>Sign Up <ArrowRight className="w-3.5 h-3.5" /></>}
          </button>
        </form>
        ...
        <div className="mt-6 text-center text-[11px] font-bold">
          ...
          <Link href="/login" className="text-primary hover:underline underline-offset-4 decoration-2 transition-all">
            Sign In
          </Link>
        </div>
      </div>
