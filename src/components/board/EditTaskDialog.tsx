...
import { Task, UserProfile } from '@/types/board';
import { useRef } from 'react';

interface EditTaskDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (taskId: string, updates: { name: string; description: string; assignedTo?: string | null }) => Promise<void>;
  task: Task | null;
  collaborators: UserProfile[];
}

const EditTaskDialog: React.FC<EditTaskDialogProps> = ({ isOpen, onClose, onSave, task, collaborators }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await onSave(task._id, { name, description, assignedTo: assignedTo || null });
      onClose();
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-md z-[100] flex items-center justify-center p-4 animate-in fade-in">
      <div className="bg-[#0f172a] w-full max-w-sm p-6 rounded-2xl shadow-2xl relative border border-white/10 animate-in zoom-in duration-300 text-slate-200">
        ...
        <h2 className="text-xl font-bold mb-4 tracking-tight text-slate-200">Edit Task</h2>
